...
export const fetchCoursesByPrice = async (min, max) => {
  let result = [];
  try {
    const response = await apiConnector(
      "GET",
      `${COURSES_BY_PRICES_API}?$minPrice=${min}&maxPrice=${max}`
    );
    // console.log("COURSE_CATEGORIES_API RESPONSE............", response)
    if (!response?.data?.success) {
      throw new Error("Could Not Fetch Course Courses by price");
    }
    result = response?.data?.data;
  } catch (error) {
    console.log("COURSE_BY_PRICES_API ERROR............", error);
    toast.error(error.message);
  }
  return result;
};
...
export const fetchCategoryPageDetails = async (
  ...
  try {
    const url =
      minPrice && maxPrice
        ? `${CATALOGPAGEDATA_API}?minPrice=${minPrice}&maxPrice=${maxPrice}`
        : CATALOGPAGEDATA_API;
    const response = await apiConnector("POST", url, { categoryId });
