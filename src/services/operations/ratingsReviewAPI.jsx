...
export function createRatingsReviews(data,token){
  return async() => {
    try {
      const response = await apiConnector("POST",CREATE_RATINGS_REVIEWS_API,data,{
...
export function updateRatingsReviews(data,token){
  return async() => {
    try {
      const response = await apiConnector("POST",UPDATE_RATINGS_REVIEWS_API,data,{
        Authorization:`Bearer ${token}`
      })
...
export const getRatingReviewSpecificCourse = async(courseId,token) => {
  let result;
  try {
    const response = await apiConnector("POST",REVIEWS_SPECIFIC_COURSE_USER_API,courseId,{
      Authorization:`Bearer ${token}`
    })
...
export const getAllRatingsReview = async() => { 
  let result;
  try {
    const response = await apiConnector("GET",GET_ALL_REVIEWS_DETAILS_API,null)
...
export const getAllRatingsReviewCourse = async(courseId) => { 
  ...
  try {
    ...
    const response = await apiConnector("POST",GET_RATINGS_REVIEWS_COURSE_SPECIFIC_API,courseId)
