...
const Card = ({heading,body}) => {
    ...
    return (
        <div className={` ${active ? "bg-white shadow-yellow-50 shadow-bottom-right2 cursor-pointer": "bg-richblack-800"} 
            h-80 w-11/12 sm:h-[21rem] md:h-80 lg:w-1/3 flex flex-col justify-between px-10 pt-10 pb-6 md:pt-7 md:pb-2 md:px-5 relative rounded-lg`
        }
            onMouseOver={handleMouseOver}
            onMouseOut={handleMouseOut}
        >
            <div className='flex flex-col gap-2'>
                <h1 className={`${active ? "text-richblack-800": "text-richblack-25"} text-xl`}>{heading}</h1>
                <p className={`${active ? "text-richblack-500": "text-richblack-400"} text-base`}>{body}</p>
            </div>
