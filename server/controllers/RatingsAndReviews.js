  ...
  try {
    ...
    if (!courseDetails) {
      return res.json({
        success: false,
        message:
          "Student is not enrolled in the course, can't review the course.",
      });
    }
    // check if user has already reviewed or not
    const ratingReview = await Rating_Reviews.findOne({
      user: userId,
      course: courseId,
    });
...
exports.getRatingReviewSpecificCourse = async (req, res) => {
  try {
    ...
    const courseSpecificReview = await Rating_Reviews.findOne({
      user: userId,
      course: courseId,
    });
...
exports.getAllRatingReview = async (req, res) => {
  try {
    console.log("getAllRatingReview");
