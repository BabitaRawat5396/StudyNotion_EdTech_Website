...
exports.categoryPageDetails = async (req, res) => {
  try {
    ...
    const { minPrice, maxPrice } = req.query; // <-- get from query

    // Filter based on price if provided
    const priceFilter = {};
    if (minPrice) priceFilter.price = { ...priceFilter.price, $gte: minPrice };
    if (maxPrice) priceFilter.price = { ...priceFilter.price, $lte: maxPrice };

    // Get courses for the specified category
    const selectedCategory = await Category.findById(categoryId)
      .populate({
        path: "courses",
        match: { status: "Published", ...priceFilter },
        populate: [
          {
